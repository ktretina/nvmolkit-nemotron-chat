...
`deployment/compose.yaml` defines one GPU-enabled application service on port 8000. The multi-stage image runs the locked frontend tests/build, installs the exact CUDA PyTorch and nvMolKit versions, serves the static UI through FastAPI, and runs as a non-root user.
